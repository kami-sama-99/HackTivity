...
export default function RegForm({ mode }) {
  ...
  const [redirecting, setRedirecting] = useState(false); // Track if redirect is in progress

  // Effect to handle redirection when loading is complete
  useEffect(() => {
    if (!loading && redirecting) {
      setTimeout(() => {
        console.log("Redirecting to:", onboarding ? "/dashboard" : "/onboarding");
        router.push(onboarding ? "/dashboard" : "/onboarding");
      }, 2000); // Delay redirection by 2 second
    }
  }, [loading, onboarding, redirecting, router]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {redirecting ? (
        <p className="text-center text-gray-400">Redirecting...</p> // Show loading text during timeout
      ) : (
        <>
          <input
            name="email"
